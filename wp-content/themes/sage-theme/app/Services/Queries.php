...
class Queries
{
    ...
    public static function getMember($page = 1)
    {
        ...
        foreach ($member->posts as $mem) {
            ...
            $mem->link_home = get_field('link_home', $mem->ID);
        }

        return $member;
    }

    public static function getMemberIn4($page = 1)
    {
        $args = [
            'post_type' =>  'member',
            'paged' => $page
    ...
    public static function getResource($page = 1)
    {
        $args = [
            'post_type' =>  'resource',
            'paged' => $page
        ...
        foreach ($resource->posts as $source) {
            $source->project = get_field('project', $source->ID);
